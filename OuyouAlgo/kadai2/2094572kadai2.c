...
int solve(int w, int h) {
    ...
    while(qSize(&strQ) > 0) {
        ...
        north = move(here, directions[3]); 
        south = move(here, directions[2]);
        west  = move(here, directions[1]);
        east  = move(here, directions[0]);

        if (canGo(here, directions[3]) == 1 && length[north.y][north.x] == 0)
        {
            if(north.x == w-1 && north.y == h-1) return length[here.y][here.x]+1;
            length[north.y][north.x] = length[here.y][here.x]+1;
            enqueue(&strQ, north);
        }

        if (canGo(here, directions[2]) == 1 && length[south.y][south.x] == 0)
        {
            if(south.x == w-1 && south.y == h-1) return length[here.y][here.x]+1;
            length[south.y][south.x] = length[here.y][here.x]+1;
            enqueue(&strQ, south);
        }

        if (canGo(here, directions[1]) == 1 && length[west.y][west.x] == 0)
        {
            if(west.x == w-1 && west.y == h-1) return length[here.y][here.x]+1;
            length[west.y][west.x] = length[here.y][here.x]+1;
            enqueue(&strQ, west);
        }

        if (canGo(here, directions[0]) == 1 && length[east.y][east.x] == 0)
        {
            if(east.x == w-1 && east.y == h-1) return length[here.y][here.x]+1;
            length[east.y][east.x] = length[here.y][here.x]+1;
            enqueue(&strQ, east);
        }
