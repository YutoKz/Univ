...
public class GUIChat {
        ...
        try {
            ...
            while(true) {
                ...
                if (msg == null) {
                    ...
                    if (strMsg.equals("reset")) { board.resetPressed(); }
                    else { displayMsgOnBoard(msg.sender, msg); }
                } else { board.recvPoints((ArrayList<ArrayList<Integer>>) msg.body); }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void sendPoints(ArrayList<ArrayList<Integer>> points) {
        assert(SwingUtilities.isEventDispatchThread());
        if(comm==null) {
            displaySysMsg("not connected.");
        }
